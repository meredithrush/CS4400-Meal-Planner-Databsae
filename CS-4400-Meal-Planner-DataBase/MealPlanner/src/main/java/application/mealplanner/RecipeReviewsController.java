...
public class RecipeReviewsController {
    ...
    public void writeReview(ActionEvent event) throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(MPApp.class.getResource("WriteReview.fxml"));
        Scene scene = new Scene(fxmlLoader.load());
        stage = getCurrentStage(event);
        stage.setScene(scene);
        WriteReviewController newController = fxmlLoader.getController();
        newController.setLoginInfo(loginInfo);
        newController.setRec_ID(rec_ID);
    }
